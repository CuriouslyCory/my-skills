...
- **branch slug**: derive a short kebab-case slug from the title if one is not given (e.g. `#128` "PatientSharedDocumentReference + PDF in the SHL bundle" becomes `128-shl-docref`).
...
```
#126 PDF export        blocked by: none
#127 schema posture    blocked by: none
#128 SHL DocRef        blocked by: #126, #127
#130 SHL to GCS        blocked by: #128
#129 IPS + US Core     blocked by: #127
```
...
Note #129 lands in Wave 2 even though a human-written serial path might place it last. That is the parallelism the wave derivation is meant to surface. #128 and #129 would both be flagged for a shared-surface check if they touch the same accessor.
